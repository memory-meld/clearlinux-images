...
kernel="vmlinux.bin"
cmdline="console=ttyS0 root=/dev/vda2 rw rootfstype=ext4,f2fs"
...
- lsblk -f;
- swupd clean;
- python3 -m pip install --upgrade --no-cache-dir drgn fire;
- lsblk -f;
EOF
...
rm -f cloudinit
mkdosfs -n config-2 -C cloudinit 131072
...
cloud-hypervisor \
  --cmdline "${cmdline}" \
  --kernel "${kernel}" \
  --disk path=root.img,direct=on \
         path=cloudinit,readonly=on \
  --cpus boot=4 \
  --memory size=12G \
  --net tap=$tap,mac=$mac \
  --serial tty \
...
status=$(ssh systemctl is-system-running --wait || true)
